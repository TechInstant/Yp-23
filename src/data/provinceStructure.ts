...
export const PROVINCE_STRUCTURE: Record<Family, ZoneNode[]> = {
  IFE: [
    {
 ...
 */
export const UNASSIGNED_PARISHES: { family: Family; name: string }[] = [
  { family: 'IFE', name: 'POWER CATHEDRAL' },
  { family: 'IFE', name: 'LIGHTHOUSE' },
]

export const OTHER = '__other__'

/** Official zones for a family, plus any extra zones already in the database. */
export function zonesFor(family: Family | '', fromDb: string[] = []): string[] {
  if (!family) return []
  const official = PROVINCE_STRUCTURE[family].map((z) => z.zone)
...
export function areasFor(family: Family | '', zone: string, fromDb: string[] = []): string[] {
  if (!family || !zone) return []
  const official =
    PROVINCE_STRUCTURE[family].find((z) => z.zone === zone)?.areas.map((a) => a.area) ?? []
...
export function flattenDirectory(): DirectoryParish[] {
  ...
  for (const family of Object.keys(PROVINCE_STRUCTURE) as Family[]) {
    for (const zone of PROVINCE_STRUCTURE[family]) {
      for (const area of zone.areas) {
        for (const name of area.parishes) {
          out.push({ name, family, zone: zone.zone, area: area.area })
        }
      }
    }
  }
  for (const p of UNASSIGNED_PARISHES) {
    out.push({ name: p.name, family: p.family, zone: '', area: '' })
  }
  return out
}
